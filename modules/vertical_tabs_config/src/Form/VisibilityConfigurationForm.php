...
class VisibilityConfigurationForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($ct_list as $ct_machine_name => $obj) {
      ...
      $form['hide_' . $ct_machine_name] = array(
        '#weight' => 5,
        '#type' => 'details',
        '#title' => $obj->get('name'),
      ...
      $form['hide_' . $ct_machine_name]['config'] = array(
        '#type' => 'fieldset',
        '#collapsible' => FALSE,
        '#collapsed' => TRUE,
      );

      $form['hide_' . $ct_machine_name]['config']['desc'] = array(
        '#type' => 'item',
        '#markup' => $this->t("Select all vertical tabs that will be hidden for @ct. If you don't select any role, vertical tabs will be hidden to all roles.", array('@ct' => $obj->get('name'))),
      );

      $form['hide_' . $ct_machine_name]['config']['roles'] = array(
        '#type' => 'fieldset',
      ...
      $form['hide_' . $ct_machine_name]['config']['tabs'] = array(
        '#type' => 'fieldset',
      ...
      foreach ($roles as $rid => $value) {
        ...
        $form['hide_' . $ct_machine_name]['config']['roles']['role_' . $ct_machine_name . '_' . $rid] = array(
          '#type' => 'checkbox',
      ...
      foreach ($vertical_tabs as $vt_machine_name => $vt_human_name) {
        ...
        $form['hide_' . $ct_machine_name]['config']['tabs']['hide_' . $ct_machine_name . '_' . $vt_machine_name] = array(
          '#type' => 'checkbox',
  ...
  public function verticalTabsConfigSaveConfig(array $config) {
    $query = Database::getConnection()->insert('vertical_tabs_config')->fields(array('vertical_tab',
      'content_type',
      'roles',
      'hidden',
      )
