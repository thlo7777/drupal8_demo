...
class OrderConfigurationForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['vttable'] = array(
      '#type' => 'table',
      '#header' => array(t('Vertical tab'), t('Weight')),
    ...
    foreach ($vertical_tabs as $vt_machine_name => $vt_human_name) {
      ...
      $form['vttable'][$id]['label'] = array(
        '#plain_text' => $vt_human_name,
      );

      // TableDrag: Weight column element.
      $form['vttable'][$id]['weight'] = array(
        '#type' => 'weight',
        '#title' => t('Weight for @title', array('@title' => $vt_human_name)),
        '#title_display' => 'invisible',
        '#default_value' => $config->get('vertical_tabs_config_' . $vt_machine_name),
        // Classify the weight element for #tabledrag.
        '#attributes' => array('class' => array('vttable-order-weight')),
      );
    }
